...
class MessageHandler:
    ...
    async def handleIncomingMessage(cls, message, sender_info, session):
        ...
        if message and handle.message_type == "text":
            if "hola" in handle.message_body.lower().strip() and len(handle.message_body.lower().strip()) <= 12:
                ...
                await cls.handle_menu.sendMenuDesert(handle.message_from)
            elif "volver" in handle.message_body.lower().strip():
                await cls.sender_message.sendReactivateMessage(handle.message_from)
                await cls.handle_menu.sendMenuConfirm(handle.message_from)
            else:
                await cls.sender_message.sendCurrentMessage(handle.message_from)
                await cls.handle_menu.sendMenuDesert(handle.message_from)
